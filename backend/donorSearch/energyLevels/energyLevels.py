...
def searchEnergyLevel(dataList:any,filterData:any):
    finalResult = []
    unique_values = dataList["energyLevelName"].unique()
    ...
    filtered_df = dataList[dataList['energyLevelName'].isin(UniquefinalMatched_results)]
    for matched_str in matched_results_with_Percent:
      filtered_df.loc[filtered_df['energyLevelName']== matched_str["Name"], 'SearchRatio'] = matched_str["Ratio"]
    finalResult = filtered_df.values.tolist()            
    return finalResult


def searchDiet(dataList:any,filterData:any):
    finalResult = []
    unique_values = dataList["DietName"].unique()
    unique_list = unique_values.tolist()
    matched_results_with_Percent = []
    finalMatched_results = []
    UniquefinalMatched_results = []
    for row in filterData:
      ...
      for matched_row in matched_results:        
        ...
        if(not exists):
          ...
          matched_results_with_Percent.extend([{'Name':matched_row[0],'Ratio':matched_row[1]}])
    UniquefinalMatched_results = list(set(finalMatched_results))    
    filtered_df = dataList[dataList['DietName'].isin(UniquefinalMatched_results)]
    for matched_str in matched_results_with_Percent:
      filtered_df.loc[filtered_df['DietName']== matched_str["Name"], 'SearchRatio'] = matched_str["Ratio"]
    finalResult = filtered_df.values.tolist()            
    return finalResult